...
class MainWindow(QWidget):
    ...
    def init_form(self):
        self.edit.setPlaceholderText("输入图片中的数字")
    ...
    def click_ok_event(self):
        if self.edit.text() == "":
            QMessageBox.warning(
                self, "警告", "请输入图片中的数字！", QMessageBox.Yes, QMessageBox.Yes)
            return
        img_name = self.img_name_list[self.idx]
        text = self.edit.text()
        ts = str(int(time.time()))
        final_img = text + "_" + ts + ".png"

        src_img = os.path.join(self.src_path, img_name)
        dst_img = os.path.join(self.dst_path, final_img)
